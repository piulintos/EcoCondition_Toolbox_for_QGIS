# -*- coding: utf-8 -*-
"""
Tool “Calculate ecosystem condition” for EcoCondition Toolbox
"""
...
class EcoCondTool:
    ...
    def setup_weights_tab(self):
        ...
        # ---- reset state‐level overrides so every run is fresh ----
        # maps spinbox.id -> overridden weight
        ...
        for state, entries in groups.items():
            # 4a) Create the state‐header item (no weight text any more)
            parent = QTreeWidgetItem([state])
            font = parent.font(0)
            font.setBold(True)
            ...
            # 4b) Insert a spin‐box into column 1 to let the user edit this state’s weight
            sb_state = QDoubleSpinBox()
            sb_state.setRange(0.05, 0.95)
            sb_state.setSingleStep(0.001)
            sb_state.setDecimals(3)
            sb_state.setValue(default_state_w)
            sb_state.setStyleSheet("background-color: lightgrey;")
            # bold text
            font = sb_state.font()
            font.setBold(True)
            sb_state.setFont(font)
            # connect into our redistributor
            sb_state.valueChanged.connect(
                lambda val, sb=sb_state: self._on_state_weight_changed(sb, val)
            )
            tree.setItemWidget(parent, 1, sb_state)
    ...
    # Check the weights for the ecosystem states
    def _on_state_weight_changed(self, sb, new_val):
        """
        Locks the edited state‐weight spinbox at new_val and
        redistributes the remaining (1 - sum(locked)) equally
        among the other, unlocked state spinboxes.
        """
        # 1) mark this spinbox as explicitly overridden
        self._state_overrides[id(sb)] = new_val

        # 2) compute total locked weight
        locked_sum = sum(self._state_overrides.values())
        locked_sum = min(max(locked_sum, 0.0), 1.0)

        # 3) compute remainder and list of free spinboxes
        remainder = 1.0 - locked_sum
        # gather all state spinboxes
        all_sbs = [
            self.dlg.treeWeights.itemWidget(
                self.dlg.treeWeights.topLevelItem(i), 1
            )
            for i in range(self.dlg.treeWeights.topLevelItemCount())
        ]
        free_sbs = [x for x in all_sbs if id(x) not in self._state_overrides]

        # 4) if no free boxes, just normalize the locked ones so they sum to 1
        if not free_sbs:
            for sb_locked in all_sbs:
                val = sb_locked.value()
                sb_locked.blockSignals(True)
                sb_locked.setValue(val / locked_sum if locked_sum else 0.0)
                sb_locked.blockSignals(False)
            return

        # 5) distribute evenly among free spinboxes
        share = remainder / len(free_sbs)
        for sb_free in free_sbs:
            sb_free.blockSignals(True)
            sb_free.setValue(share)
            sb_free.blockSignals(False)
    ...
    def calculate_weighted_sums(self):
        ...
        for state in EC_STATES:
            ...
            if not items:
                continue
            parent = items[0]
            # grab the spin‐box widget in column 1 instead of the empty text cell
            sb_state = tree.itemWidget(parent, 1)
            if isinstance(sb_state, QDoubleSpinBox):
                state_w = sb_state.value()
            else:
                # fallback, just in case
                txt = parent.text(1)
                state_w = float(txt) if txt else 0.0
